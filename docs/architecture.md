...
graph TD
    ...
    subgraph "Future Integration"
        E["Notification Channel e.g. Slack / Email"]
    end
    
    D -->|"Forwarded by Log Agent e.g. Fluentd"| E
...
-   The **Python Consumer Pod** starts up, scans the `incoming/` directory for any files, processes them, generates reports, and then terminates.
...
#### 5. Future Integration (Notifications)
The logs captured by Kubernetes can be easily integrated with other systems.
-   A logging agent like **Fluentd** or **Promtail** can be deployed to the cluster to automatically collect logs from all pods.
-   These agents can then forward the logs to external systems, such as a monitoring dashboard (like Grafana Loki) or a notification channel (like Slack or email), allowing for automated alerting if a job fails.
